...
% wykład 14.05.2013
\begin{theorem}[Herbrant]
Niech $S$ - zbiór klauzul. $S$ jest spełnialny wtedy i tylko wtedy, gdy $S$ jest spełnialny w modelu Herbranta.
\begin{proof}[$\Rightarrow$]
\[ I = \lbrace~p (t_1, \ldots, t_k) \in B_L : p (t_1, \ldots, t_k) \text{ jest prawdziwy w modelu } M~\rbrace \]
\[ \bar{M} = (U_L, I) \text{ jest modelem Herbranta dla zbioru } S \]
...
\begin{theorem}
$I, J$ - dwa modele Herbranta dla programu $P$. Postać ich klazul to:
\[ C = \forall_{x_1,\ldots,x_k} A \lor \lnot B_1 \lor \ldots \lor \lnot B_n \]
$I \cap J$ - jest modelem Herbranta dla programu $P$.
\end{theorem}

\begin{theorem}
Niech $\alpha = \lbrace I_i \rbrace$ - niepusta rodzina modeli Herbranta dla $P$, to $\bigcap I_i$ jest modelem dla $P$.
\begin{proof}
...
Niech $\alpha$ - rodzina wszystkich modeli Herbranta dla programu $P$. Zauważmy, że $\alpha \neq \emptyset$, ponieważ należy tam model
...
\begin{enumerate}
  ...
  \item $P \cup \lbrace \lnot A \rbrace$ nie ma modelu Herbranta
  \item $\lnot A$ jest fałszywe w każdym modelu Herbranta dla programu $P$
  \item $A$ jest prawdziwe w każdym modelu Herbranta dla programu $P$
  \item $A \in I$ dla każdego $I$, będącego modelem Herbranta dla programu $P$
...
$a$ jest \emph{supermum} dla $X$, gdy jest najmniejszym ograniczeniem górnym $X$. Oznaczenie: $\sqcup X = a$ - $a$ jest supremum dla $X$.
