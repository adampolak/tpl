...
\begin{theorem}[O dedukcji niewprost, TDN]
...
\begin{proof}
 ~\begin{itemize}
    \item $K \in L'$:
      \begin{flalign*}
        a &\in Cu_{L'}(\{a,b\}) = Cu_{L'}(\{a\} \cup \{b\}) &&
        \text{oczywiste} \\
        b \impl a &\in Cu_{L'}(\{a\}) && \text{z TDW} \\
        a \impl (b \impl a) &\in Cu_{L'}(\emptyset) = L' && \text{z TDW}
      \end{flalign*}
    \item $S \in L'$:
      \begin{flalign*}
        a &\in Cu_{L'}(\{a\impl (b\impl c), a \impl b, a\}) &&
        \\text{oczywiste} \\
        b &\in Cu_{L'}(\{a\impl (b\impl c), a \impl b, a\}) &&
        \text{MP na }a\impl b\text{ i }a \\
        b \impl c &\in Cu_{L'}(\{a\impl (b\impl c), a \impl b, a\}) &&
        \text{MP na }a\impl (b \impl c)\text{ i }a \\
        c &\in Cu_{L'}(\{a\impl (b\impl c), a \impl b, a\}) &&
        \text{MP na }b \impl c\text{ i }b \\
        (a \impl c) &\in Cu_{L'}(\{a\impl (b\impl c), a \impl b\}) &&
        \text{z TDW} \\
        (a\impl b) \impl (a \impl c) &\in Cu_{L'}(\{a\impl (b\impl c)\}) &&
        \text{z TDW} \\
        (a\impl (b\impl c)) \impl ((a\impl b) \impl (a \impl c)) &\in
        Cu_{L'}(\emptyset) = L' && \text{z TDW} 
      \end{flalign*}
  \end{itemize}
\end{proof}
...
\begin{proof}
  ~\begin{itemize}
    \item W~$L'$ zachodzi TDW:
      \begin{itemize}
        \item Jeśli $b\impl a \in Cu_{L'}(X)$ to $a\in Cu_{L'}(X\cup\{b\})$
          zachodzi w~każdym zbiorze zamkniętym na MP.
        \item Jeśli $a\in Cu_{L'}(X\cup\{b\})$ to $b\impl a \in Cu_{L'}(X)$  
          zachodzi, bo:
          \begin{flalign*}
            a &\in Cu_{L'}(X\cup\{b\}) \subset Cu_{L'}(X\cup\{b,\neg a\})
            && \text{z założenia} \\
            \neg a &\in Cu_{L'}(X\cup\{b,\neg a\}) && \text{oczywiste} \\
            b \impl a &\in Cu_{L'}(X) && \text{z TDN}
          \end{flalign*}
      \end{itemize}
    \item $N \in L'$:
      \begin{flalign*}
        \neg b, b &\in Cu_{L'}(\{\neg a \impl \neg b,\neg a \impl b, \neg a\})
        && \text{dwa razy MP} \\
        (\neg a \impl b) \impl a &\in Cu_{L'}(\{\neg a \impl \neg b\})
        && \text{z TDN} \\
        (\neg a \impl \neg b) \impl ((\neg a \impl b) \impl a)
        &\in Cu_{L'}(\emptyset) && \text{z TDW}
      \end{flalign*}
  \end{itemize}
\end{proof}
...
%sapal: W moich notatkach jest, że prawo Pierce'a jest takie, jak napisałem
%       (a internet się z tym zgadza). Nie wiem, jak te dwie rzeczy mają się
%       do siebie (mogą być równoważne).
