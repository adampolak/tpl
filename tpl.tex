...
\begin{itemize}
  ...
  \item Gdy chcemy skonstruować $\lambda$-term dla formuły
    ...
    \begin{align*}
      ...
      &\lambda x_{(A \impl B) \impl A}. x(\lambda y_{A\impl B}. \square_{A} ) \\
      &\lambda x_{(A \impl B) \impl A}. x(\lambda y_{A\impl B}. y
      (\lambda z_{A}. \square_{B})) \\
    \end{align*}
    Nie mamy czego wpisać w~ostatni kwadracik: nie da się uzyskać zmiennej typu
    $B$ (podobnie jak nie ma skąd wziąć zmiennej typu $A$, żeby użyć jej
    w~przedostatniej linijce). Wynika to z~tego, że prawa Pierce'a nie da się
    udowodnić w~logice intuicjonistycznej.
\end{itemize}
